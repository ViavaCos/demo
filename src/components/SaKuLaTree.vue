...
export default {
    ...
    methods: {
        initTree() {
            ...
            this.drawTree(tree.width/2,this.rootTop,-Math.PI/2,30,1);//执行
            requestAnimationFrame(this.step);
        },

        step (){
            var len = this.flowerList.length ;
            if(Math.random() > 0.3)    this.fallList.push(this.flowerList[Math.floor(Math.random()*len)]);//随机取出一个，花瓣复制到飘落花瓣的列表中

            this.cxt.clearRect(0,0,tree.width,tree.height);
            for(var i = 0 ;i < this.fallList.length ; i ++){
                if(this.fallList[i].sy < this.limitSpeedY) this.fallList[i].sy += this.g ;
                this.fallList[i].sx += this.gWind ;
                this.fallList[i].x += this.fallList[i].sx ;
                this.fallList[i].y += this.fallList[i].sy ;
                if(this.fallList[i].y > this.rootTop){//飘到树根的花瓣移除
                ...
                this.cxt.arc(this.fallList[i].x,this.fallList[i].y,this.fallList[i].r,this.fallList[i].deg,this.fallList[i].deg+Math.PI*1.3);
                this.cxt.fill();
            }
            requestAnimationFrame(this.step);
        },
        ...
        drawTree (x,y,deg,step,type) {
           var deg1 = step%2 == 0 ? 0.1 : -0.1 ;
            ...
            if(step < 3 || (step < 23 && Math.random() > 0.1)){
                //末梢位置 画花瓣
                var color = [this.flowerColorDeep,this.flowerColor,this.flowerColor][Math.round(Math.random()+0.2)] ;
                var r = 2+Math.random()*2
                this.tCxt.fillStyle = color ;
                this.tCxt.arc(x1+Math.random()*3,y1+Math.random()*3,r,0,Math.PI)
                this.tCxt.fill();
                this.flowerList.push({x:x,y:y,sx:(Math.random()-0.5),sy:0,color:color,r:r,deg:deg});//保存下画樱花的位置
            }
            step -- ;
            if(step > 0){
                this.drawTree(x1,y1,deg,step,type);
                if(step%3 == 1 && step > 0 && step < 30)
                    this.drawTree(x1,y1,deg+0.2 + 0.3 * Math.random(),Math.round(step/1.13));//右分叉
                if(step%3 == 0 && step > 0 && step < 30)
                    this.drawTree(x1,y1,deg-0.2 - 0.3 * Math.random(),Math.round(step/1.13));//左分叉
            }
        }
    },
